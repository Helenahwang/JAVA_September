...
@WebServlet("*.file") //인덱스에 있는 .file 인 것을 읽으려고 
public class FileController extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		case "fileupload.file":
			...
			dispatcher.forward(request, response); //forward : 데이터 없이 이동 redirect : 데이터를 이동시키려면 
		...
		case "upload.file":
			...
			//fileuploadService interface로 이동 => fileuploadServiceImple에서 구체화
			
			//처리 결과를 저장
			request.setAttribute("map", map);
			
			/*
			//처리 결과를 출력할 페이지로 이동 // 이 경우 새로고침하면 같은 파일을 새롭게 계속 저장된다. 
			  => 따라서 리다이렉트로 해야 한다.  
			...
			*/
			...
			response.sendRedirect("uploadresult.file"); // sendRedirect : 업로드 한번만 실행한다.
			// 결과 페이지("views/result.jsp")의 이름을 그대로 쓰면 안된다. 
			//따라서 "uploadresult.file" 라고 쓰고 아래에서 case 문으로 해서 포워딩 시킨다.
		...
		case "insert.file":
			...
			if(result == true) {
				response.sendRedirect("./"); //디렉토리가 없는 메인 페이지로 index.jsp
